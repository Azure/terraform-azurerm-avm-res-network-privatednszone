...
variable "records" {
  ...
  validation {
    condition     = alltrue([for r in var.records : length(r.value) > 0 && alltrue([for v in r.value : length(v) > 0])])
    error_message = "Each TXT record must have a non-empty value list, and all values must be non-empty strings."
  }
}
